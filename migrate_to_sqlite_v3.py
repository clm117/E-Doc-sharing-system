    ...
    try:
        ...
        if rows:
            ...
            for row in rows:
                # 根据返回的字段数量确定如何处理
                if len(row) >= 26:
                    record_id, code, msg, sub_code, sub_msg, trade_no, out_trade_no, buyer_id, buyer_logon_id, seller_id, seller_email, total_amount, receipt_amount, invoice_amount, buyer_pay_amount, point_amount, refund_fee, subject, body, gmt_create, gmt_payment, gmt_refund, gmt_close, file_id, create_time, update_time = row[:26]
                    # 转换日期格式
                    ...
                    # 基本字段
                    record_id, code, trade_no, out_trade_no, buyer_id, buyer_logon_id, seller_id, total_amount, subject, gmt_create, file_id = row[:11]
                    # 其他字段设为默认值
                    msg = None
                    sub_code = None
                    sub_msg = None
                    ...
                    # 最少字段
                    record_id, trade_no, out_trade_no, buyer_id, total_amount, subject, file_id = row[:7]
                    # 其他字段设为默认值
                    code = '10000'
                    msg = 'Success'
                    sub_code = None
