...
def encrypt_all_files_in_directory():
    try:
        print("正在连接数据库...")
        connection = cx_Oracle.connect(**DB_CONFIG)
        cursor = connection.cursor()
        
        print("开始加密目标目录下的所有PDF文件...")
        print("=" * 80)
        ...
        for i, filename in enumerate(pdf_files):
            ...
            try:
                # 读取PDF文件（使用strict=False来处理损坏的文件）
                ...
                # 记录损坏的文件
                if "EOF marker not found" in error_msg or "endstream" in error_msg or "Stream has ended" in error_msg:
