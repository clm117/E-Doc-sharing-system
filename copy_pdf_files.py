            ...
            try:
                ...
                if os.path.exists(target_path):
                    ...
                    if source_size == target_size:
                        skipped_count += 1
                        print(f"⏭️  [{i+1}/{len(source_files)}] 已存在且大小相同，跳过：{filename}")
                        continue
                    else:
                        # 删除旧文件
                ...
                success_count += 1
                print(f"✅ [{i+1}/{len(source_files)}] 复制成功：{filename}")
                
                if (i + 1) % 100 == 0:
                    print(f"\n已处理{i+1}/{len(source_files)}个文件...")
                
            except Exception as e:
                failed_count += 1
                print(f"❌ [{i+1}/{len(source_files)}] 复制失败：{filename}, 错误：{str(e)}")
