...
# -*- coding: utf-8 -*-
"""
从Excel文件B列读取文件名，生成随机密码和分类，导入到file_info表
"""
...
# 文件分类选项
FILE_MAJOR_CLASSES = ['科技', '心理', '医学', '法律', '数学', '英语', '历史', '会计']
FILE_MID_CLASSES = ['C语言', 'python', 'java', '前端', '数据库']
FILE_MINOR_CLASSES = ['爬虫', '大数据', '人工智能']
...
def main():
    ...
    try:
        ...
        INSERT INTO file_info (
            ...
            file_major_class, file_mid_class, file_minor_class
        ) VALUES (
            :file_id, :file_name, :file_password, 
            :file_major_class, :file_mid_class, :file_minor_class
        )
        ...
        for i, file_name in enumerate(file_names, 1):
            ...
            # 随机选择文件分类
            file_major_class = random.choice(FILE_MAJOR_CLASSES)
            file_mid_class = random.choice(FILE_MID_CLASSES)
            file_minor_class = random.choice(FILE_MINOR_CLASSES)
            ...
            data = {
                ...
                'file_major_class': file_major_class,
                'file_mid_class': file_mid_class,
                'file_minor_class': file_minor_class
            }
        ...
        # 提交剩余的记录
        connection.commit()
        print(f"数据导入完成，共插入{len(file_names)}条记录")
