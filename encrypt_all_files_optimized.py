...
def encrypt_all_files_optimized():
    try:
        print("正在连接数据库...")
        connection = cx_Oracle.connect(**DB_CONFIG)
        cursor = connection.cursor()
        
        print("开始加密所有PDF文件（优化版）...")
        print("=" * 80)
        ...
        for i, filename in enumerate(pdf_files):
            ...
            try:
                ...
                with open(filepath, 'rb') as f:
                    ...
                    for page_num in range(page_count):
                        try:
                            ...
                            print(f"⚠️  [{i+1}/{len(pdf_files)}] 页面{page_num}读取失败：{filename}")
                            continue
                ...
                # 记录损坏的文件
                if "EOF marker not found" in error_msg or "endstream" in error_msg or "Stream has ended" in error_msg or "PdfReadError" in error_msg or "PdfStreamError" in error_msg or "RecursionError" in error_msg or "maximum recursion depth" in error_msg:
                    corrupt_files.append(filename)
                    print(f"⚠️  [{i+1}/{len(pdf_files)}] 文件损坏，跳过：{filename}")
                else:
                    print(f"❌ [{i+1}/{len(pdf_files)}] 加密失败：{filename}, 错误：{error_msg[:100]}")
        ...
        if corrupt_files:
            print("\n损坏的PDF文件列表：")
            for i, filename in enumerate(corrupt_files[:20]):
                print(f"  {i+1}. {filename}")
            if len(corrupt_files) > 20:
                print(f"  ... 还有{len(corrupt_files)-20}个文件")
