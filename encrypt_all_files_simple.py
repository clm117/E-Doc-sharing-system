...
def encrypt_all_files_simple():
    try:
        print("正在连接数据库...")
        connection = cx_Oracle.connect(**DB_CONFIG)
        cursor = connection.cursor()
        
        print("开始加密所有PDF文件（简化版）...")
        print("=" * 80)
            ...
            try:
                ...
                with open(filepath, 'rb') as f:
                    ...
                    if reader.is_encrypted:
                        print(f"⏭️  [{i+1}/{len(pdf_files)}] 已加密，跳过：{filename}")
                        continue
                ...
                failed_count += 1
                print(f"❌ [{i+1}/{len(pdf_files)}] 加密失败：{filename}, 错误：{str(e)[:100]}")
