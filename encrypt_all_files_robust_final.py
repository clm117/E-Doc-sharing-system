...
def encrypt_all_files_robust():
    try:
        print("正在连接数据库...")
        connection = cx_Oracle.connect(**DB_CONFIG)
        cursor = connection.cursor()
        
        print("开始加密所有PDF文件（增强版）...")
        print("=" * 80)
        ...
        for i, filename in enumerate(pdf_files):
            ...
            try:
                # 读取PDF文件（使用strict=False来规避EOF错误）
                ...
                # 记录损坏的文件（EOF marker not found等错误）
                if "EOF marker not found" in error_msg or "endstream" in error_msg or "Stream has ended" in error_msg or "PdfReadError" in error_msg or "PdfStreamError" in error_msg:
