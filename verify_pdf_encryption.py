            ...
            try:
                ...
                with open(filepath, 'rb') as f:
                    ...
                    if reader.is_encrypted:
                        encrypted_count += 1
                        print(f"✅ [{i+1}/{min(20, len(pdf_files))}] 已加密：{filename}")
                    else:
                        not_encrypted_count += 1
                        print(f"❌ [{i+1}/{min(20, len(pdf_files))}] 未加密：{filename}")
            
            except Exception as e:
                error_count += 1
                print(f"⚠️  [{i+1}/{min(20, len(pdf_files))}] 检查失败：{filename}, 错误：{str(e)}")
